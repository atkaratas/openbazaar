export default function HomePage() {
  return (
    <div className="flex flex-col items-center justify-center">
      ...
      {/* Hero Banner - Gıda Odaklı (Zeytin Yeşili / Sıcak Tonlar) */}
      <section className="w-full bg-emerald-700 text-white py-20 px-4 text-center">
        ...
        <div className="flex justify-center gap-4">
          <button className="bg-white text-emerald-700 font-bold py-3 px-8 rounded-full shadow-lg hover:bg-gray-100 transition-all">
            Lezzetleri Keşfet
          </button>
      ...
      <section className="max-w-7xl mx-auto py-16 px-4 w-full">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {[
            { name: 'Zeytinyağı & Soslar', icon: '🫒' },
            { name: 'Kuruyemiş & Atıştırmalık', icon: '🌰' },
            { name: 'Geleneksel Tatlılar', icon: '🍯' },
            { name: 'Baharat & Otlar', icon: '🌶️' }
          ].map((cat, i) => (
            <div key={i} className="bg-white h-48 rounded-2xl shadow-sm border border-emerald-100 flex flex-col items-center justify-center text-gray-700 hover:shadow-md hover:border-emerald-300 transition-all cursor-pointer">
